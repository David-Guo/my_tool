...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None):
        ...
        createMode = action('Create\nRectBox', self.setCreateMode,
                'Ctrl+Z', 'new', u'Start drawing Boxs', enabled=False)
        editMode = action('&Edit\nRectBox', self.setEditMode,
                'Ctrl+J', 'edit', u'Move and edit Boxs', enabled=False)

        create = action('Create\nRectBox', self.createShape,
                'Ctrl+Z', 'new', u'Draw a new Box', enabled=False)
        ...
        fitWidth = action('Fit &Width', self.setFitWidth,
                ...
                checkable=True, enabled=False)
        #neohe
        loadXml = action('&Load xml', self.loadXML,
                'Ctrl+Shift+L', 'load-xml', u'load xml',
    ...
    def editLabel(self, item=None):
        ...
        text ="corner"# self.labelDialog.popUp(item.text())
        if text is not None:
            item.setText(text)
            self.setDirty()
    ...
    def newShape(self):
        ...
        text ="corner"# self.labelDialog.popUp()
        if text is not None:
    ...
    #neohe
    def loadXML(self,value=True):
        if value:
            self.openAnnotation(value)
    #end for neohe
    ...
    def loadFile(self, filename=None):
        print("""Load the specified file, or the last opened file if None.""")
        self.resetState()
        ...
        if QFile.exists(filename):
            ...
            print("label file name is :",self.defaultSaveDir)
            if self.labelFile:
            ...
            ## neohe default loading xml
            basename = os.path.basename(os.path.splitext(self.filename)[0]) + '.xml'
            path=os.path.split(os.path.realpath(filename))[0]
            print("basename :",basename)
            #print("filename :",filename)
            xmlPath = os.path.join(path,'../gen_xml', basename)
